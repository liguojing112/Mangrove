<template>
  <div class="relative min-h-screen bg-gray-50 pb-[96px]">
    <!-- 背景装饰：五线谱纹理 + 浮动音符 -->
    <div class="absolute inset-0 pointer-events-none select-none z-0" aria-hidden="true">
      <svg class="absolute top-0 left-0 w-full h-full opacity-[0.08]" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="staff-lines-music" x="0" y="0" width="100%" height="150" patternUnits="userSpaceOnUse">
        ...
        <rect width="100%" height="100%" fill="url(#staff-lines-music)"/>
      </svg>
    ...
    <!-- Swiper -->
    <section class="relative z-10 mb-5 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16" aria-label="音乐精选">
      <Swiper
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <template v-else>
        ...
        <div v-for="song in filteredSongs" :key="song.id" class="song-row flex items-center gap-3 border-b border-gray-100 px-2 py-3 transition-all duration-200 hover:bg-green-100 hover:shadow-sm hover:shadow-green-500/10 hover:-translate-y-0.5"
